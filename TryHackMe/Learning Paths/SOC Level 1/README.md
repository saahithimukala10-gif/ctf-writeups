# TryHackMe — Learning Paths: SOC Level 1

Running notes for the TryHackMe SOC Level 1 path, organized by section. Each linked file is a short key-points/revision note for that topic, same treatment as [Pre Security](../Pre%20Security/README.md) — not a full writeup, since these are conceptual/role-overview rooms rather than hands-on exploitation. Hands-on SOC rooms (Splunk, Wireshark, Snort, capstone challenges, etc.) will get full writeups in [`Challenges.md`](../../Challenges/Challenges.md) as they're completed.

## Section 1 - Blue Team Introduction

- [Junior Security Analyst Intro](Section%201%20-%20Blue%20Team%20Introduction/Junior%20Security%20Analyst%20Intro.md)
- [SOC Role in Blue Team](Section%201%20-%20Blue%20Team%20Introduction/SOC%20Role%20in%20Blue%20Team.md)
- [Humans as Attack Vectors](Section%201%20-%20Blue%20Team%20Introduction/Humans%20as%20Attack%20Vectors.md)
- [Systems as Attack Vectors](Section%201%20-%20Blue%20Team%20Introduction/Systems%20as%20Attack%20Vectors.md)

**4 topics completed — Section 1: Blue Team Introduction complete, moving on to Section 2: SOC Team Internals.**

## Section 2 - SOC Team Internals

- [SOC L1 Alert Triage](Section%202%20-%20SOC%20Team%20Internals/SOC%20L1%20Alert%20Triage.md)
- [SOC L1 Alert Reporting](Section%202%20-%20SOC%20Team%20Internals/SOC%20L1%20Alert%20Reporting.md)

**6 topics completed — in progress on Section 2: SOC Team Internals.**
